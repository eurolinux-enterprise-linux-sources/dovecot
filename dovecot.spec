...
# fix issues and assert crashes found in 2.0.9, from upstream, for dovecot < 2.0.10
Patch4: dovecot-2.0.9-regrfix.patch

# for dovecot < 2.0.13
Patch5: dovecot-2.0-CVE-2011-1929.patch

# for dovecot < 2.0.10, rhbz#697620
Patch6: dovecot-2.0.9-c75a13e5b678.patch

# for dovecot < 2.0.16, rhbz#754980, upstream commits: 5e9eaf63a6b1,de8715e4d793
Patch7: dovecot-2.0.16-CVE-2011-4318.patch

# for dovecot < 2.0.13, rhbz#709095
Patch8: dovecot-2.0.13-CVE-2011-2166.patch

# for dovecot < 2.0.14, rhbz#709097
Patch9: dovecot-2.0.13-CVE-2011-2167.patch

# rhbz@1010279

Patch10: dovecot-2.0.9-fipsfix.patch
Patch11: dovecot-2.0.9-CVE_2014_3430.patch
Patch12: dovecot-2.0.9-copyhang.patch

# for dovecot < 2.0.17, rhbz#771336
Patch13: dovecot-2.0.9-gsssize.patch

# for dovecot <2.1, rhbz#1153041
Patch14: dovecot-2.0.9-sslprot.patch
Patch15: dovecot-2.0.9-stopfix.patch

# not yet upstream, rhbz#1026790, rhbz#1100756
Patch16: dovecot-2.2.9-nodevrand.patch

# for dovecot <2.1, rhbz#1184865
Patch17: dovecot-2.0.9-slowcreate.patch

# for dovecot <2.0.11, rhbz#730746
Patch18: dovecot-2.0.9-stderrclose.patch

# for dovecot <2.0.16, rhbz#813957
Patch19: dovecot-2.0.9-noproxycrl.patch
...
Requires: initscripts portreserve
...
%patch4 -p1 -b .regrfix
%patch5 -p1 -b .CVE-2011-1929
%patch6 -p1 -b .c75a13e5b678
...
%patch14 -p1 -b .sslprot
%patch15 -p1 -b .stopfix
%patch16 -p1 -b .nodevrand
%patch17 -p1 -b .slowcreate
...
#install portreserve file
install -p -D -m 644 %{SOURCE15} $RPM_BUILD_ROOT%{_sysconfdir}/portreserve/%{name}
...
if ! grep -q ssl_protocols %{_sysconfdir}/dovecot/conf.d/10-ssl.conf
then
cat >>%{_sysconfdir}/dovecot/conf.d/10-ssl.conf <<EOF
...
* Wed Mar 04 2015 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-18
- enable tcp_wrappers (#1124783)

* Thu Feb 26 2015 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-17
- fix race condition when creating INBOX (#1184865)
- close stderr if it's not used for logging (#730746)
- login proxy should not fail connection if CRL isn't found for some certificate (#813957)
- fix crash in list-status (#961466)
- installation of dovecot hangs if there is no /dev/urandom (#1026790)

* Mon Feb 23 2015 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-16
- workaround glibc issue, use portreserve (#723228)

* Thu Feb 19 2015 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-15
- installation of dovecot in FIPS mode hangs (#1100756)

* Tue Feb 17 2015 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-14
- ALL in ssl_protocols did not match TLSv 1.1 and 1.2 (#1182619)

* Tue Feb 17 2015 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-13
- wait for services to stop listening before unlinking the pid file (#1140459)

* Tue Dec 16 2014 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-12
- fix post install script (#1153041)

* Fri Dec 12 2014 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-11
- mention it in config file, that we disabled SSLv2 and SSLv3
  by default (#1153041)
- add ssl_protocols configuation option and disable SSLv2 and SSLv3
  by default (#1153041)

* Mon Dec 08 2014 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-10
- dovecot had too small LOGIN_MAX_INBUF_SIZE for GSSAPI (#771336)

* Wed Nov 05 2014 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-9
- do not hang after UID COPY for nonexiting mailbox (#1131749)

* Wed Jun 11 2014 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-8
- fix CVE-2014-3430: denial of service through maxxing out SSL connections (#1108002)

* Mon Oct 07 2013 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-7
- relro protection disappeared from some files, bring it back

* Mon Oct 07 2013 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-6
- ssl-params got stuck in FIPS mode (#1010279)

* Mon Dec 10 2012 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-5
...
* Tue Oct 02 2012 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-4
- dovecot may not set correct premissions for mail folder (#697620)

* Mon Aug 08 2011 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-3
- fix potential crash when parsing header names that contain NUL characters (#728673)

* Mon Jan 31 2011 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.9-2
...
* Thu Jan 13 2011 Michal Hlavinka <mhlavink@redhat.com> - 1:2.0.8-1
